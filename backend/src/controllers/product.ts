...
export const newProduct = TryCatch(async (req: Request<{}, {}, NewProductRequestBody>, res: Response, next: NextFunction) => {
  ...
  if (!name || !price || !stock || !category || !description) return next(new ErrorHandler("Please enter All Fields", 400));
...
export const updateProduct = TryCatch(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (photos && photos.length > 0) {
    ...
    photosURL.forEach((photo) => {
      product.photos.push(photo as any);
    });
  }
...
export const allReviewsOfProduct = TryCatch(async (req: Request, res: Response, next: NextFunction) => {
...
export const newReview = TryCatch(async (req: Request, res: Response, next: NextFunction) => {
